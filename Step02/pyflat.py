...
class ElementTRUSS:
    ...
    def calc_stiffness_matrix(self) -> MatrixType:
        return calc_stiffness_matrix_truss(
            self.get_coord_matrix(),
            self.property.young,
            self.property.area)

    def calc_result(self, ans_elem: VectorType) -> list[VectorType]:
        return calc_result_truss(
            self.get_coord_matrix(),
            self.property.young,
            self.property.area,
            ans_elem)
...
def main():
    ...
    #   全体剛性行列の構築
    # ******************************************************
    matrix_shape: linalg.IShape = linalg.ScipyDenseShape.get_instance(neq)
    ...
    for label, elem in enumerate(all_elems, start=1):
        ...
        results: list[VectorType] = elem.calc_result(ans_elem)
