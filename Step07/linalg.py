...
class ISolver(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def add_spc(self, idof: int, value: float, rhs: VectorType) -> None:
        ...
...
class ScipyDenseSolver(ISolver):
    ...
    def add_spc(self, idof: int, value: float, rhs: VectorType) -> None:
        self.__matrix[idof, idof] *= PENALTY_FACTOR
        rhs[idof] = self.__matrix[idof, idof] * value
        return
