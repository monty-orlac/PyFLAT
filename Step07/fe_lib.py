...
def _shape_func_1d(nedge: int, xi: float) -> VectorType:
    match nedge:
        case 2:
...
def _shape_derivs_1d(nedge: int, xi: float) -> VectorType:
    match nedge:
        case 2: return np.array([-0.5, 0.5])
        case 3: return np.array([xi - 0.5, xi + 0.5, -2.0 * xi])
        case _: pass
...
def calc_edge_plane2d(coords: MatrixType, thickness: float, value: float, direction: tuple[float, float] | None = None) -> VectorType:
    ...
    for xi, weight in _gauss_info_1d(nedge):
        ...
        # 1次元形状関数 N1, N2
        slocal: VectorType = _shape_func_1d(nedge, xi)

        # 形状関数の自然座標微分 dN/dxi
        dslocal: VectorType = _shape_derivs_1d(nedge, xi)
        ...
        norm_vector: VectorType = np.zeros(2)
...
def get_element_edge_ids(nnode: int, iedge: int) -> cabc.Sequence[int]:
    match nnode:
        case 3: return [iedge, (iedge + 1) % 3]
        case 4: return [iedge, (iedge + 1) % 4]
        case 6: return [iedge, (iedge + 1) % 3, iedge + 3]
        case _: pass
    raise AssertionError(f"Unknown nnode{nnode}")
