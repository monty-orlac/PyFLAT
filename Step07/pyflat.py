...
def main():
    ...
    all_conds: list[list[fe.ConditionBase]] = [
        [
            fe.PointLoad(all_nodes[2], 1, 10000.0),
            fe.PointLoad(all_nodes[3], 1, 10000.0),
        ], [
            fe.EdgeLoad(all_elems[2], 0, -4000.0, None),
        ], [
            fe.Boundary(all_nodes[2], 1, 0.100),
            fe.Boundary(all_nodes[3], 1, 0.100),
        ], [
            fe.PointLoad(all_nodes[1], 0, -10000.0),
            fe.PointLoad(all_nodes[2], 0,  10000.0),
            fe.PointLoad(all_nodes[2], 1,  10000.0),
            fe.PointLoad(all_nodes[3], 0,  10000.0),
            fe.PointLoad(all_nodes[3], 1, -10000.0),
        ], [
            fe.EdgeLoad(all_elems[0], 0, 4000.0, (-1.0,  0.0)),
            fe.EdgeLoad(all_elems[2], 0, 4000.0, (+1.0,  0.0)),
            fe.EdgeLoad(all_elems[1], 0, 4000.0, ( 0.0, +1.0)),
            fe.EdgeLoad(all_elems[3], 0, 4000.0, ( 0.0, -1.0)),
        ], [
            fe.Boundary(all_nodes[1], 0, 0.000),
            fe.Boundary(all_nodes[2], 0, 0.100),
            fe.Boundary(all_nodes[3], 0, 0.100),
            fe.Boundary(all_nodes[2], 1, 0.000),
            fe.Boundary(all_nodes[3], 1, 0.000),
        ],
    ]
    ...
    for ilcase, conds in enumerate(all_conds, start=1):
        ...
        for cond in conds:
            if isinstance(cond, fe.ConditionLoad):
                cond.apply(rhs)
            elif isinstance(cond, fe.ConditionSPC):
                (dof, value) = cond.apply()
                matrix_solver.add_spc(dof, value, rhs)
        ...
        print("Loadcase #{:02d}".format(ilcase))
